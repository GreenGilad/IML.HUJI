...
\todo{This algorithm is easily adapter for online learning}\\
...
\begin{algorithm}
	...
	\begin{algorithmic}
		...
		\While{not converged}
		\For{$i=1,\ldots,m$}
		\If{$\y_i \neq sign\left(\x_i^\top\w^{\left(t\right)}\right)}$ \Comment{If predicted wrong}
		\State $\w^{\left(t+1\right)} \gets \w^{\left(t\right)} + \y_i\x_i$ \Comment{Update parameters}
		\EndIf
		\EndFor
		\EndWhile
		\State \textbf{return} $\w^{\left(t+1\right)}$
		\EndProcedure
	\end{algorithmic}
\end{algorithm}
