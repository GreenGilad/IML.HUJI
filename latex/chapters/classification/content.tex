...
\begin{figure}[h!]
	\centering
	\includegraphics[width=2in]{chapters/classification/figures/boundry1.png}
	\includegraphics[width=2in]{chapters/classification/figures/boundry2.png}
	\caption{\textbf{Decision Boundaries} of a $\left(A\right)$ linear classifier and a $\left(B\right)$ $k$-nearest neighbors classifier}
\end{figure}
...
\\~\\
This trade-off was first studied during World War II, when radar was invented. The designer of the radar had to choose when to put a green dot on the radar, indicating a target detected there. Sometime radar waves would bounce off back from clouds or birds, and the designer had to choose a \textbf{threshold} $\alpha$. If the radar pulse returning is stronger than $\alpha$, the radar screen would show a green dot. If weaker than $\alpha$, no dot. Now, if $\alpha$ is set too low (say $\alpha=0.1$), the screen would be full of a thousand green dots - since any bird or cloud (with, say, $h(\x=0.2$) would be classified as \textbf{positive}, a target. So that the radar will be full of false positives, false targets, and will be useless. On the other hand, if $\alpha$ is set too high (say $\alpha=0.9$) then enemy airplanes  (with, say, $h(\x=0.8$) will not appear on the screen, since they will be classified by mistake as birds, and the radar again would be useless. 
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=2in]{chapters/classification/figures/roc.png}
	\caption{\todo{Create own graph + caption}}
\end{figure}
...
\todo{Matan - Add content for Perceptron}
...
\\~\\
But what if the training sample is almost linearly separable? That is, what if most of the samples are linearly separable with only a few violating the constraints by ``not too much``? Recall that if $y_i\cdot\left(\inprod{\w}{\x_i}+b\right)<0$ then sample $\x_i$ is on the ``wrong side`` of the hyperplane. This means that: $$\exists \,\xi_i> 0\quad s.t.\quad y_i\cdot\left(\inprod{\w}{\x_i}+b\right) \geq 1-\xi_i $$ Therefore, sample $\x_i$ is on the wrong side of the \textbf{margin} by an amount proportional to $\xi_i$. To allow training samples to violate the constraints ``a little``, we modify the optimization problem to:
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.3\textwidth]{chapters/classification/figures/soft_svm.png}
\end{figure}
~\\
...
\subsubsection{The Kernel Method}\label{kernel_method}

\todo{Go over this part - Instead of being a subsubsection set as section. Begin with motivation. Explain computability problem. Formally define valid kernel functions. Prove polynomial and Gaussian Kernels. Example of 2-order poly kernel. Show how to think of Gaussian kernel as similarity!. Show (without proving) some closure properties. Then solve SVM using kernels.}\\

The three classifiers seen above (halfspace classifier, Hard-SVM and Soft-SVM) all use the halfspaces hypothesis class. Though these linear classifiers are fairly simple to implement, the hypothesis class of halfspaces is of limited power. Consider the case where $\Xc=\R$ with the following sample:
\begin{figure}[H]
	\centering
	\includegraphics{chapters/classification/figures/linearly_inseparable.png}
\end{figure}
As the data in not linearly separable, all three learners will fail. Notice however, that if instead of using the original feature representation of the data, we embed the samples in a new feature space, such as mapping each sample $x\rightarrow\left(x,x^2\right)$, we get a linearly separable sample in $\R^2$:
\begin{figure}[H]
	\centering
	\includegraphics{chapters/classification/figures/linearly_separable.png}
\end{figure}

Therefore, to enrich the expressiveness of the halfspace hypothesis class, we can consider embedding the data into another (potentially of higher dimension) feature space, over which we then learn a halfspace. Schematically:
\begin{itemize}
	\item Given some domain set $\Xc\subseteq\R^d$ and a learning problem, select an embedding  $\psi:\Xc\rightarrow\Fc$ for some feature space $\Fc$. In most cases $\Fc\subseteq \R^k$ for some $k>d$.
	\item Train a halfspace using the training set $\left\{\left(\psi\left(\x_i\right),y_i\right)\right\}^m_{i=1}$.
\end{itemize}
~\\
\todo{move to bottom and properly show how to use kernels for svm} Applying the above scheme to the Soft-SVM learner we would therefore want to solve:
$$
\underset{\w,b}{\min}\left(\lambda \norm{\w}^2 +\frac{1}{m}\sum\ell^{hinge}\left(y_i\left(\inprod{\psi\left(\x_i\right)}{\w}+b\right)\right) \right)
$$
This approach seems very attractive as the assumption that our data is linearly separable in \textit{some} feature space does not look very limiting. However, several challenges need to be addressed:
\begin{itemize}
...
To address the computational complexity problem, we could use kernel based learning. Some algorithms do not directly depend on the data $\psi\left(\x_i\right)$, but rather access the data only through the result of the dot-product $\inprod{\psi\left(\x_i\right)}{\psi\left(\x_j\right)}$. In these cases we might be able to find an efficient way to compute the result $\inprod{\psi\left(\x_i\right)}{\psi\left(\x_j\right)}$ without ever directly computing  $\psi\left(\x_i\right)$. This approach is also known as the \textit{kernel trick}, and the embedding function $\psi$ is a kernel function.


\begin{claim}[Polynomial Kernels]
The following mapping, of $k$ degree polynomials, is a kernel function: $$ K\left(\x,\x'\right)\coloneqq \left(1+\inprod{\x}{\x'}\right)^k $$
\end{claim}
\begin{proof}
To show the above is indeed a kernel function let us find a mapping $\psi$ such that $K\left(\x,\x'\right)=\inprod{\x}{\x'}$. For simplicity let $x_0=1=x'_0$. Then: 
$$
\begin{array}{ccl}
...
\end{array} $$
By defining $\psi$ as $\psi\left(\x\right)=\left(1,1\cdot x_1, x_1 \cdot 1, 1\cdot x_2, x_2\cdot 1, x_1^2, x_2^2, x_1\cdot x_2, x_2 \cdot x_1\right)^\top$ we achieve that $K\left(\x,\x'\right)=\inprod{\x}{\x'}$
\end{example}

%\begin{claim}[Gaussian Kernels]
%	The following mapping is a kernel function: $$ K\left(\x,\x'\right)\coloneqq \exp\left(-\frac{1}{2}\norm{\x-\x'}^2\right) $$
%\end{claim}
%\begin{proof}
%%Let us define the embeding function $\psi$ such that $\psi\left(\x\right)$
%\end{proof}


By embedding a given sample using a polynomial kernel of degree $k$, and learning a halfspace in this embedding, we are in-fact learning a polynomial predictor of degree $k$ over the original space.

In the case of polynomial kernels we used an embedding in $\R^{\left(n+1\right)^k}$. 
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.5\textwidth]{chapters/classification/figures/logistic.png}
	\caption{\todo{todo}}
\end{figure}
...
\todo{Do we want to add explanations about the Newton-Raphson iterations?}
...
\begin{enumerate}
	\item Prove that optimization problem \ref{eqn:hard_svm_opt3} is a Quadratic Program.
\end{enumerate}