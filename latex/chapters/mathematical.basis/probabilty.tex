...
\begin{definition}
	...
	$X$ is
	\[
	\bE\left[X\right]=\sum_{x\in\mathcal{X}}x\Dc\left(x\right)\qquad\mbox{or}\qquad \bE\left[X\right]=\int_{-\infty}^{\infty}xf\left(x\right)dx
	\]
	
\end{definition}
Properties:
\begin{itemize}
	\item $\bE\left[g\left(X\right)\right]=\sum g\left(x\right)\Dc\left(x\right)$,
	...
	since $\bE[XY]=\sum\Dc(X=x,Y=y)x\cdot y = \sum_{x,y}\Dc(x)\Dc(y)xy=\bE[X]\bE[Y]$.
\end{itemize}
...
\begin{itemize}
	...
	\item $V(aX+b)=a^{2}V(X)$
	\item $V(X+Y)=\bE\left[\left(X+Y-\bE(X)-\bE(Y)\right)^{2}\right]=V(X)+V(Y)+2\overbrace{\bE\left[\left(X-\bE[X]\right)\left(Y-\bE[Y]\right)\right]}^{\sigma \left(X,Y \right)}$
...
\end{definition}
$$
\sigma(X,Y)\equiv \sigma_{XY}\equiv=\bE\left[\left(X-\bE[X]\right)\left(Y-\bE[Y]\right)\right]=\bE\left[XY\right]-\bE[X]\bE[Y]
$$
...
\begin{example}
	...
	\begin{eqnarray*}
		\bE\left[X\right] & = & \frac{1}{b-a}\int_{a}^{b}xdx=\frac{b+a}{2}\\
		\bE\left[X^{2}\right] & = & =\frac{b^{2}+ab+a^{2}}{3}\\
