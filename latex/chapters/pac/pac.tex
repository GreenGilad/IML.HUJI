...
\vspace{3mm}
{\bf Question:} Is there a strategy that will guarantee some upper bound $0\leq\epsilon<1$ on the loss with probability 1 (namely, almost surely for any sample $S$ that might be
	drawn according to $\Dc$?) 
	%     \item Is there a learner $\Ac$ (``strategy we play'') that will ensure 
	%        $L_{\Dc,f}(h) = 0$ 


\textbf{Answer}: {\bf No.} Choose $0\leq \epsilon <1 $. The learner cannot hope to produce,
regardless of how Nature plays, and with probability $1$, a rule $h$ with $L_{\Dc,f}(h) \leq \epsilon$.
...
{\bf Example: }
		Take $\Xc=\left\{ x_1,x_2 \right\}$.  Fix $\gamma$. Our learner $\Ac$ must specify what to predict on a point that was not seen in the training set. WLOG we choose to predict $+$ on a point we have not seen in the training set. Now, Nature plays $\Dc$ with
		...
		As as result, on a random test point drawn according to $\Dc$, unfortunately $h_S$ will be wrong with probability $1-\gamma$. So
		$L_{\Dc,f}(h)=(1-\gamma)$. Now make $\gamma$ small enough to have $1-\gamma
		> \epsilon$. 
 ...
 As $\epsilon$ was arbitrary, this means that for any arbitrarily "bad" loss $\epsilon$, Nature can cause the game to end, with strictly positive probability, with a loss at least $\epsilon$.
	  What went wrong? In this example, with probability $\gamma^m$ (which is really very tiny) we get a "lousy" training sample $S$, one that does not represent $\Dc$ and good enough, and does not allow us to generalize. 
	  
 
  {\bf Claim:} For any desired upper bound $\epsilon<1$ on the loss, 
	no learner $\Ac$ can guarantee $L_{\Dc,f}(h_S) \le
	\epsilon$ with probability $1$ against {\bf any} strategy of Nature. 
	
  {\bf Relaxation:} We allow the learner $\Ac$ to ``fail completely''
