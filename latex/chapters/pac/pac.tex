...
The framework in Section \ref{sec:frame} can be thought of as a {\bf game} between us and Nature, with a random payoff. The game proceeds as follows. The number of training samples $m$ is given in advanced as a game parameter. \\
...
Recalling that the loss is random - it depends on the draw of training sample
$S$ - we now can ask the following questions. 

\vspace{3mm}
{\bf Question:} Given an $0\leq\epsilon<1$, is there a strategy that will guarantee that the loss will not exceed $\epsilon$, with probability 1? In other words, that almost surely, for any sample $S$ drawn according to $\Dc$, we will have  $L_{\Dc,f}(h) \leq \epsilon$?
	%     \item Is there a learner $\Ac$ (``strategy we play'') that will ensure 
	%        $L_{\Dc,f}(h) = 0$ 


\textbf{Answer}: {\bf No.} 
Choose $0\leq \epsilon <1 $. The learner cannot hope to produce, regardless of how Nature plays, and with probability $1$, a rule $h$ with $L_{\Dc,f}(h) \leq \epsilon$.

\textbf{Reason}: There's always a (small) probability to get a completely ``pathological'' training	sample $S$ that does not represent $\Dc$ at all. The resulting rule $h_S$ can
	be wrong on most of $\Xc$. If $S$ is really bad, $h_S$ can have a loss as high as $1$, higher than any $\epsilon$.
	
{\bf Example: }
		Choose $0\leq \epsilon <1 $. Take $\Xc=\left\{ x_1,x_2 \right\}$.  Fix $0<\gamma$ such that  $1-\gamma>\epsilon$. Our learner $\Ac$ must specify what to predict on a point that was not seen in the training set. 
		Let us assume, without a loss of generality, that our algorithm predicts the label $+1$ on a point we have not seen in the training set. Now, Nature chooses $\Dc$ with
		$\Dc(\{x_1\}) = 1-\gamma$, $\Dc(\{x_2\}) = \gamma$, and chooses a labeling function $f$ with  $f(x_1)=f(x_2)=-1$. 
		
		With probability 		$\gamma^m$, the training sample $S$ contains only  $x_2$'s. How does $h_S=\Ac(S)$
		classify $x_1$ in this event? Since there's no information in the training data about $x_1$, the algorithm will predict $h_S(x_1)=+1$.
		As as result, on a random test point drawn according to $\Dc$, unfortunately $h_S$ will be wrong with probability $1-\gamma$, which is the probability to draw $x_1$. Thus,
		$L_{\Dc,f}(h)=1-\gamma>\epsilon$. Now make $\gamma$ small enough to have $1-\gamma > \epsilon$. 
 In conclusion , even on a sample space $\Xc$ with only two points, for any fixed $\epsilon>0$, 
		for every strategy $\Ac$ we might play, Nature has a strategy $\Dc,f$, such that 
		with some probability $\delta$ that depends only on $\epsilon$, over the choice of training samples of length $m$, we have  
		$L_{\Dc,f}(h_S) \geq \epsilon$.
 ...
 As $\epsilon$ was arbitrary, this means that for any arbitrarily "bad" loss $\epsilon$, Nature can cause the game to end, with strictly positive probability, with a loss at least $\epsilon$.
	  What went wrong? In this example, with probability $\gamma^m$ (which is really very tiny) we get a "lousy" training sample $S$, one that does not represent $\Dc$  good enough, and does not allow us to generalize. 
	  
 
  {\bf Conclusion:} No learner $\Ac$ can guarantee, that given an $0<\epsilon<1$, and with probability $1$,  the loss will have an upper bound of  $L_{\Dc,f}(h_S) \le \epsilon$, against \textit{any} strategy of Nature. 
	
  {\bf Relaxation:} We allow the learner $\Ac$ to ``fail completely''	(produce $h_S$ with potentially huge loss) with
	probability $\delta$, where $\delta \in (0,1)$ is specified in the conditions 	of the game.
	
  {\bf Bottom line:} We must allow a small probability for
	``pathological'' sample $S$, from which it is impossible to generalize. 
	
  {\bf Definition:} When a learner $\Ac$ has  an arbitrary large loss with probability
...
\textbf{Question:} In the event of a non-pathological sample (the event with probability at least $1-\delta$), can we please have perfect accuracy?  
In other words, is there a strategy $\Ac$ for us with {\bf zero loss} (w.p. at least $1-\delta$) regardless of how Nature plays?
	%     \item Is there a learner $\Ac$ (``strategy we play'') that will ensure 
	%        $L_{\Dc,f}(h) = 0$ 
	
\textbf{Answer:} {\bf No.} Choose $\delta>0$ (probability of complete failure - getting an
	arbitrarily large loss). The learner cannot 	hope to produce, regardless of how Nature plays, and with probability at
	least $1-\delta$, a 	rule $h$ with $L_{\Dc,f}(h_S)=0$.
	
	\textbf{Reason:} $L_{\Dc,f}(h)=0$ means that $\Dc\{h_S(x)= f(x)\}=1$ ($h_S$ is never wrong, almost surely with respect to $\Dc$). 	If Nature plays $\Dc$ that gives a
	tiny mass to some $x\in\Xc$, with high probability, the 	$S$ will not include $x$, and there, $h_S$ has no 	idea what to do. So it cannot generalize perfectly to $x$.
	...
	\begin{itemize}
		...
		As before, fix $\gamma$. Our learner $\Ac$ must specify what to predict on a point that was not seen in the training set. WLOG we choose to predict $+$ on a point we have not seen in the training set. As before, Nature plays $\Dc$ with
		$\Dc(\{x_1\}) = 1-\gamma$, $\Dc(\{x_2\}) = \gamma$, and chooses a labeling function $f(x_1)=f(x_2)=-$. 
		...
		As as result, on a random test point drawn according to $\Dc$, unfortunately $h_S$ will be wrong with probability $\gamma$. So
		$L_{\Dc,f}(h)=\gamma>0$. Now make $\gamma$ small enough to have $(1-\gamma)^m > \delta$.
		...
		$L_{\Dc,f}(h_S)$ strictly positive and bounded away from $0$.
	...
	{\bf Claim:} For any confidence $\delta$, 
	...
	{\bf Relaxation:} We must be satisfied with an {\bf approximately correct}
	learning rule: A rule $h_S$ with $L_{\Dc,f}(h_S) \le
	\epsilon$, where $\epsilon$ is specified in the conditions of the game.
	
	{\bf Bottom line:} Even a ``typical'' (``non-pathological'') training sample $S$ may miss small areas in $\Xc$. On these areas
    ...
    {\bf Definition:}  When a learner $\Ac$ has an upper bound $\epsilon>0$ on
...
\begin{definition}
	...
	probability at least $1-\delta$, for some $\delta>0$ and $\epsilon>0$, 
...
If $S$ is by chance "weird" (not representing $\Dc$ well), the rule $h$ produced will be "wrong", namely, it won't generalize well. The number $\delta$ is the probability of failure due to a "weird" sample $S$.
\textit{Second}, we test the rule $h$ on new data. New data is also random. $L_{\Dc,f}(h)$ is the expected number of errors $h$ will make. The number $\epsilon$ refers to the accuracy of $h$.
