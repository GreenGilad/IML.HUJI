...
~\\ The eigenvalues of $A$, $\lambda_1\geq\ldots\geq\lambda_d$ are referred to as the \textbf{Principal Values} of $\X$. The eigenvectors of $A$, $\vv{u}_1,\ldots,\vv{u}_d$ are referred to as the \textbf{Principal Vectors}. Notice that the matrix $A$ is a $d$-by-$d$ positive semi-definite matrix. Therefore \todo{finish}
...
In Figure todo{add reference} we can see the difference between the projection and the coordinates of the data-points. Figure {add reference to left side} we see a dataset in $\R^d$ and the two dimensional subspace found by the PCA algorithm. The orthogonal projection of the points ...........
