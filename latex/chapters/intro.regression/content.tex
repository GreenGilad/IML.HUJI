...
In the case of linear regression we aim to model the relation using a linear predictor function $f$. Formally, the hypothesis class of linear regressors is the set of linear functions from the domain set to the response set: 
...
\subsection{A Statistical Model - Existence of Noise}
...
\todo{Matan - Regression via projections - with noise $\widehat{\y}$ no longer in image}
...
\section{Polynomial Fitting} %TODO - fix psi font
...
\todo{Add graphs for polynomial fitting?}
...
The $\ols$ estimator is a random variable (as $\y$ is a random variable) and as such we are also interested in different properties of it.

\todo{If the math chapter includes estimation theory -> move the definitions there}
...
\\~\\
Going back to the $\ols$ estimator, we could ask what is it's bias and variance.
...
& = & \E\left[\w\right] + \left[\X^\top\X\right]^{-1}\X^\top\E\left[\eps\right]\\
& = & \w
\end{array}
$$
...
The aim of the linear regression lab is to explore the linear regression hypothesis class and specifically the OLS estimator. 
