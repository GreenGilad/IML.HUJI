...
\begin{figure}[!h]
	\centering
	\includegraphics[width=0.3\textwidth]{chapters/intro.regression/figures/regression_noiseless_intuition.png}
	\caption{\textbf{Probabilistic Model:} \todo{add caption}}
\end{figure}
...
Using the tools of linear regression, we could also learn non-linear predictors. In fact, it can be used to find predictors that are a linear combination of some set of functions $\phi_0,\ldots,\phi_k$. Consider the following special case of polynomial fitting: Let $x_1,\ldots,x_m\in\R$ and $y_1,\ldots,y_m\in\R$. We would like to describe a polynomial relation between $\Xc$ and $\Yc$, of degree at most $k\in\N$. The hypothesis class is: 
\begin{equation}
\Hc^k_{poly}=\left\{x\mapsto p_{\w}\left(x\right)| \w\in\R^{k+1}\right\}
\label{eqn:poly_reg}
\end{equation}
where $p_{\w}\left(x\right)=\sum^{K}_{i=1}w_i x^i$. In this case, the set of functions is given by $\phi_i\left(x\right)=x^i$ for any $i\in\left\{0,\ldots,k\right\}$. Then denote $\phi\left(x\right)\coloneqq\left(\phi_0\left(x\right),\ldots,\phi_k\left(x\right)\right)^\top$. Thus, a polynom of degree at most $k$ is given by:
$$ p_{\w}\left(x\right)=\sum^k_{i=0}w_i\phi_i\left(x\right)=\phi\left(x\right)^{\top} \w=\left\langle\phi\left(x\right),\w\right\rangle $$

As before we want to find a coefficients vector $\w$. Given a sample $\trainset$ we transform it to a sample $\widetilde{S}\coloneqq\left\{\left(\phi\left(x_i\right),y_i\right)\right\}^m_{i=1}$, and solve the following LS problem: $$ \widehat{w}\coloneqq\underset{\w\in\R^{k+1}}{argmin}\frac{1}{m}\sum^m_{i=1}\left(\phi\left(x_i\right)^\top \w -y_i\right)^2 $$
...
\left[
\begin{array}{ccc}
\horzbar & \phi\left(x_1\right) & \horzbar \\
\horzbar & \phi\left(x_2\right) & \horzbar \\
& \vdots    & \\
\horzbar & \phi\left(x_m\right) & \horzbar
\end{array}
\right] = 
\left[
\begin{array}{ccccc}
