...
\begin{equation}
R\left(\theta,\widehat{\theta}\right)\coloneqq\E_\theta\left|\theta-\widehat{\theta}\right|
\label{eqn:abs}
\end{equation}

\begin{equation}
R\left(\theta,\widehat{\theta}\right)\coloneqq\E_\theta\left(\theta-\widehat{\theta}\right)^2
\label{eqn:mse}
\end{equation}
...
Regression is a statistical method to model the relation between a set of explanatory variable (i.e features) and a scalar response, also referred to as dependent variable. Therefore, our sample domain is $\Xc=\R^d$ and our response set is $\Yc=\R$. We assume that there exists some deterministic (unknown to us) function $f:\Xc\rightarrow\Yc$ that is \textit{underlying} the relation between each sample $x$ and it's response $y$.
...
In the case of linear regression we aim to model the relation using a linear predictor function $f$. Formally, the hypothesis class of linear regressors is the set of linear functions from the domain set to the response set: $$ \Hreg\coloneqq\left\{h: h\left(x_1,\ldots,x_d\right)=w_0+\sum^d_{i=1}x_iw_i,\,\, w_0,w_1,\ldots,w_d\in\R\right\} $$
...
\subsubsection{Least Squares Loss Function} %TODO - better explanation of risk vs. loss and the empirical risk..
...
Let us show that $\widehat{\w}=\left[\X^\top\X\right]^{-1}\X^\top\y$ is the desired minimizer. Before proving so we will first make an assumption over our data $\X,\y$. We will assume that the different features (i.e columns of $\X$) are linearly independent. We will make this assumption in all of the models we will discuss in the course. %TODO - explain why it is okay to assume so?
...
As our model allows some uncertainty in the responses, even for identical $\x$s, we may end up with different $y$s. Let us adapt our learning algorithm from the deterministic case, and deal with the probabilistic (noisy) one. In the same manner as before, using the linear hypothesis class $\Hreg$,  we asusme there exists an unknown coefficients vector $\w$. Denoting the noise vector $\eps=\left(\eps_1,\ldots,\eps_m\right)^\top$ we have in matrix notation: $$ \y=\X\w+\eps, \quad\quad \eps\sim\Nc\left(0_d,\sigma^2I_d\right) $$

Note, that the vector $\y$ is no longer necessarily in $Im\left(\X\right)$. Therefore, the linear system $\y=\X\w$ has no solution. 
To derive a learning algorithm let us use the squared loss as before, and learn by empirical risk minimization as in \eqref{eqn:opt_rss}. This means learning the prediction rule by solving the normal equations, which we have seen is equivalent to projecting $\y$ onto $Im\left(\X\right)$ by finding the Least Squares approximation.

\begin{figure}[!h]
	\centering
	\includegraphics[width=0.3\textwidth]{chapters/intro.regression/figures/regression_noiseless_intuition.png}
	\caption{\textbf{Probabilistic Model:} \todo{add caption}} %TODO - image and caption
\end{figure}
...
\todo{Show geometric interpretation and solution via projection matrices}
\subsection{Categorical Variables}
\todo {Matan - Write sub-section about categorical variables}


\section{Beyond Linearity}
\subsection{Polynomial Fitting} %TODO - fix psi font
Using the tools of linear regression, we could also learn non-linear predictors. In fact, it can be used to find predictors that are a linear combination of some set of functions $\psi_0,\ldots,\psi_k$. Consider the following special case of polynomial fitting: Let $x_1,\ldots,x_m\in\R$ and $y_1,\ldots,y_m\in\R$. We would like to describe a polynomial relation between $\Xc$ and $\Yc$, of degree at most $k\in\N$. The hypothesis class is: 
\begin{equation}
$$ \Hc^k_{poly}=\left\{x\mapsto p_{\w}\left(x\right)| \w\in\R^{k+1}\right\} $$
\label{eqn:poly_reg}
\end{equation}
where $p_{\w}\left(x\right)=\sum^{K}_{i=1}w_i x^i$. In this case, the set of functions is given by $\psi_i\left(x\right)=x^i$ for any $i\in\left\{0,\ldots,k\right\}$. Then denote $\psi\left(x\right)=\left(\psi_0\left(x\right),\ldots,\psi_k\left(x\right)\right)^\top$. Thus, a polynom of degree at most $k$ is given by:
$$ p_{\w}\left(x\right)=\sum^k_{i=0}w_i\psi_i\left(x\right)=\psi\left(x\right)^{\top} \w=\left\langle\psi\left(x\right),\w\right\rangle $$

As before we want to find a coefficients vector $\w$. Given a sample \trainset we transform it to a sample $\widetilde{S}=\left\{\left(\psi\left(x_i\right),y_i\right)\right\}^m_{i=1}$, and solve the following LS problem: $$ \widehat{w}=\underset{\w\in\R^{k+1}}{argmin}\frac{1}{m}\sum^m_{i=1}\left(\psi\left(x_i\right)^\top \w -y_i\right)^2 $$

Let us notice that the design matrix $\X$ defined over the transformed data is the Vandermonde matrix:
$$
\X =
\left[
\begin{array}{ccc}
\horzbar & \psi\left(x_1\right) & \horzbar \\
\horzbar & \psi\left(x_2\right) & \horzbar \\
& \vdots    & \\
\horzbar & \psi\left(x_m\right) & \horzbar
\end{array}
\right] = 
\left[
\begin{array}{ccccc}
...
\todo{Add subsection - second round of writing}
\section{Summary}
\subsection{Exercises}
\begin{enumerate}
	\item Let $A$ be a real $n\times m$ matrix. Show that the null space of $A$ and $A^\top A$ is the same. That is: $Ker\left(A\right)=Ker\left(A^\top A\right)$.
	\item Let $A$ be a real $n\times m$ matrix. If the columns of $A$ are independent then $A^\top A$ is a positive definite matrix.
	\item Let $\X$ be a design matrix of a linear regression problem such that $\X^\top\X$ is invertible. Show that ${\X^\dagger}^\top=\left(\X^\top\X\right)^{-1}\X$.
\end{enumerate}
\todo{add questions from assignments}
