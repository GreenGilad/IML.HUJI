...
\begin{figure}[h!]
\centering
\includegraphics[width=4.5in]{chapters/ensemble.methods/figures/ESL_bias_variance.png}  
...
\todo{Add motivations for approximation and estimation - which is influeced just by S where by H and so on + end of recitation 8 page 5}

\todo{Add explanation of how to properly create bias-variance curves}
...
It's been known for a long time that committees typically make better decisions than individuals. Consider a committee of $T$ members, which has to make a yes/no decision. In hindsight we'll know whether the decision has been right or wrong. Each member casts a vote. Each one has probability $p$ of being correct and probability $1-p$ of being wrong. Let's assume for simplicity that all members are ``equally wise'', so that $p$ is the same for all members. After all members vote, the committee's decision is simply the majority vote.

\begin{exercise}
	Let $X_1,\ldots,X_T\overset{iid}{\sim}Ber\left(p\right)$ taking values of $\left\{\pm1\right\}$. What is the probability the committee will make the right decision?
\end{exercise}
\begin{proof}
...
As each random variable takes a value in $\left\{\pm\right\}$ we could express the collective vote as $\overline{X}=sign\left(\sum_{i=1}^TX_i\right)$. If the committee decided right, then ther are more members that voted right and $\sum_{i=1}^TX_i>0$ which means that $\overline{X}=1$. On the otherhand, if the committee decided wrong, then more members voted wrong and $\sum_{i=1}^TX_i<=0$ which means that $\overline{X}=-1$.

So, we conclude that: $$ \Prob\left(\text{Committee decides right}\right)=\Prob\left(\overline{X}>0\right) $$
\end{proof}

\begin{exercise}
	Let $X_1,\ldots,X_T\overset{iid}{\sim}Ber\left(p\right)$ taking values of $\left\{\pm1\right\}$ with $p>0.5$. Bound the probability of the committee being correct
\end{exercise}
\begin{proof}
	\todo{Recitation 08 - page 2}
\end{proof}
...
\todo{ Matan - Add subsection about Gradient Boosting}
...
\begin{enumerate}
	\item Let $X_1,\ldots,X_m$ be i.i.d random variables with some distribution $\mathcal{F}$ on the real line $\R$. Let $x_i\in\R$ be a sampled value of $X_i$. Show that the CDF of this sample is a step function, increasing from $0$ to $1$, with jump of size $1/m$ at each value $x_i$ (see Figure \ref{ecdf}).
	\item Given a sample $\trainset$ and a bootstrap sample $S^*$ show that for large $m$, about 37\% of data points are not in $S^*$.
	\item \todo{check and fix - Showing that transition to predicted class probabilities is an MLE estimator of it}
\end{enumerate}